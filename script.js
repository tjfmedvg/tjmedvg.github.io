document.addEventListener("DOMContentLoaded", () => {
    ...
    // Create and append the modal close button
    const closeButton = document.createElement("button");
    closeButton.innerHTML = "&times;";
    closeButton.classList.add("close-button");
    closeButton.setAttribute('aria-label', 'Close product details');
    const modalContentContainer = productDetail.querySelector('.product-detail-content');
    ...
    const showProductDetail = (product) => {
        const fallbackImgUrl = `https://placehold.co/500x500/e2e8f0/475569?text=No+Image`;
        const fallbackThumbUrl = `https://placehold.co/100x100/e2e8f0/475569?text=N/A`;

        // Set main image with a fade-in effect
        ...
        product.images.forEach((image, index) => {
            ...
            thumb.className = 'h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 object-cover rounded-lg cursor-pointer border-2 border-transparent hover:border-[var(--primary-color)] transition-all duration-200 shadow-sm hover:shadow-md';
    ...
    products.forEach(product => {
        ...
        const fallbackImgUrl = `https://placehold.co/400x400/e2e8f0/475569?text=Not+Found`;

        card.innerHTML = `
                    <div class="aspect-w-1 w-full overflow-hidden bg-gray-100 dark:bg-gray-800 relative">
                        <img src="${product.images[0]}" alt="${product.title}" class="object-cover" onerror="this.onerror=null; this.src='${fallbackImgUrl}';">
                    ...
                    <div class="p-5 flex flex-col flex-grow">
                        ...
                        <div class="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700">
                            <a href="${product.link}" target="_blank" class="buy-now-card-btn w-full block bg-gradient-to-r from-[var(--primary-color)] to-[var(--secondary-color)] hover:from-[var(--primary-hover-color)] hover:to-[var(--secondary-hover-color)] text-white font-semibold px-4 py-2.5 rounded-lg shadow-md hover:shadow-lg transition duration-300 text-center text-sm transform hover:scale-105 group">
        ...
        card.addEventListener('click', (e) => {
            // Prevent modal from opening if the "Buy Now" button itself is clicked
